...
좌측 메뉴에서 여섯 가지 생성 유형을 전환할 수 있습니다:
...
| 구성 이미지    | 구성도 이미지 분석 및 관련 문서 자동 생성 |
...
* **결과 편집 기능**
  ...
  * 기능 리스트, 테스트케이스, 결함 리포트는 생성 후 전용 편집 페이지에서 항목별 수정·추가·삭제 가능
  * 구성 이미지 편집 페이지에서 이미지 기반 분석 결과를 수정 가능
* **결함 리포트 최종 반영**
...
| **Infra**         | Docker Compose, Nginx (리버스 프록시 · HTTPS), Doppler (시크릿 관리) |
| **문서 처리**      | openpyxl, pandas, pypdf, python-docx, beautifulsoup4  |
...
* **파일 업로드·생성·편집·다운로드를 하나의 워크플로로 통합**
* **Doppler 기반 환경 변수 중앙 관리 — `.env` 수동 공유 불필요**
* **Nginx 리버스 프록시 + 자체 서명 SSL 인증서로 HTTPS 지원**
* **`server_start.sh` 원클릭 배포 스크립트 제공**
...
4. **"생성하기"** 클릭 → AI가 결과를 자동 생성
5. 전용 편집 페이지에서 결과를 검토·수정
6. **CSV 다운로드** 또는 Google 스프레드시트로 최종 반영
7. 필요 시 관리자 페이지에서 프롬프트 수정 및 재실행

---

## 📂 관련 문서

* [협업 가이드](docs/COLLABORATION_GUIDE.md) — 온보딩, 디렉터리 구조, 핵심 로직 안내
* [Doppler 사용 가이드](docs/DOPPLER_GUIDE.md) — 시크릿 관리 및 배포 가이드
