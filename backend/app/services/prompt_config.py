...
class PromptResourcesConfig(BaseModel):
    model_config = ConfigDict(
        extra="ignore", alias_generator=_to_camel, populate_by_name=True
    )
...
class PromptConfig(BaseModel):
    ...
    prompt_resources: PromptResourcesConfig | None = None
...
_DEFAULT_PROMPTS: Dict[str, PromptConfig] = {
    ...
    "testcase-workflow-scenarios": PromptConfig(
        label="테스트케이스 워크플로 - 시나리오 도출",
        summary="기능 설명을 분석하여 테스트 시나리오 후보를 JSON으로 도출합니다.",
        request_description="기능별 테스트 시나리오 후보를 JSON({\"scenarios\": [...]}) 형식으로 정리합니다.",
        system_prompt=(
        ...
        user_prompt_sections=[
            PromptSection(
                id="testcase-workflow-scenarios-guidelines",
                label="작성 지침",
                content=(
        ...
        scaffolding=PromptScaffolding(
            ...
            attachments_intro="",
            closing_note="",
            format_warning="응답은 JSON 한 개만 반환하세요.",
        ),
    ...
    "testcase-workflow-rewrite": PromptConfig(
        label="테스트케이스 워크플로 - 시나리오 다듬기",
        summary="생성된 테스트 시나리오를 대화형으로 보완하고 재작성합니다.",
        request_description="기존 테스트 시나리오와 사용자 요청을 반영하여 JSON 형식으로 수정안을 반환합니다.",
        system_prompt=(
            "당신은 소프트웨어 테스트 전문가입니다. 사용자의 테스트케이스를 개선하고 명확하게 다듬어 주세요."
        ),
        user_prompt=(
            "프로젝트 개요:\n{{project_overview}}\n\n"
            "기능 분류:\n{{feature_classification}}\n\n"
        ...
        user_prompt_sections=[
            PromptSection(
                id="testcase-workflow-rewrite-guidelines",
                label="응답 형식 지침",
                content=(
                    "- 아래 JSON 형식으로만 응답하세요.\n"
        ...
        scaffolding=PromptScaffolding(
            ...
            attachments_intro="",
            closing_note="",
            format_warning="응답은 JSON 객체 한 개만 반환하세요.",
        ),
        model_parameters=PromptModelParameters(
            temperature=0.2,
            top_p=0.9,
    ...
    "testcase-workflow-finalize": PromptConfig(
        label="테스트케이스 워크플로 - CSV 완성",
        summary="시나리오 요약을 기반으로 테스트케이스 표를 완성합니다.",
        request_description="소분류별 시나리오 요약을 CSV(열은 파이프(|)로 구분) 형식으로 정리합니다.",
        system_prompt=(
        ...
        user_prompt=(
            "프로젝트 개요:\n{{project_overview}}\n\n"
            "기능별 시나리오 요약:\n{{scenario_summary}}\n\n"
            "CSV 열은 {{csv_headers}} 순서를 따릅니다. 지침을 모두 준수해 테스트케이스를 작성하세요."
        ),
        user_prompt_sections=[
            PromptSection(
                id="testcase-workflow-finalize-guidelines",
                label="작성 지침",
                content=(
    ...
    "defect-report": PromptConfig(
        ...
        user_prompt_sections=[
            PromptSection(
                ...
                content=(
                    ...
                    "2. 결함정도는 치명/중대/보통/경미 판단 결과를 H/M/L 코드로 기입하고, 발생빈도는 Always/Intermittent 등 로그 근거를 A/I 코드로 변환해 작성하세요.\n"
                    "3. 품질특성에는 기능적합성, 신뢰성 등 관련 분류를 지정하고, 업체 응답과 수정여부는 근거 자료가 없으면 '-'로 표기합니다."
                ),
            ),
            PromptSection(
        ...
        prompt_resources=PromptResourcesConfig(
            judgement_criteria="""이미지와 결함 내용, 결함 판단 기준, 출력 예시를 참고하여 결함 리포트 행을 작성하세요.

[출력 형식 지침]
- 결함정도는 위험도에 따라 H(High) / M(Medium) / L(Low) 코드 중 하나로만 작성합니다.
- 발생빈도는 항상 발생(A) 또는 가끔 발생(I) 중 하나의 코드로만 작성합니다.
...
1. 기능 적합성 : 주요 기능이 잘 동작하는지
    ...
    3. 기능 적절성 : 기능을 적절하게 제공하는지
        - CRUD 기에서 ‘생성/추가/등록/만들기’ 등의 기능이 제공되는 경우, 추가된 객체를 조회/변경/삭제하는 기능이 제공되는지 확인
...
4. 사용성
    ...
    2. 학습성 <> 기능적합성
        - 프로그램에서 제공는 기능에 대한 설명이 매뉴얼에 기술되어 있는지 확인 → 기능 정보 미제공
...
5. 신뢰성
    ...
    2. 결함허용성
        ...
        1. Fail-over 기능 오류 - H: 이중화 기능이 활성화 된 상황에서, Active 서버 비정상 종료 시, Standby 서버로 Fail-over 되지않음
    3. 복구성
...
6. 보안성
    ...
    1. 기밀성 : Invicti에 리포팅 (치명적, 높은, 중간)
        ...
        1. 사용자 계정정보 접근통제 오류 - H: 아이디/비밀번호가 txt 파일로 저장되어, 비인가자가 해당 파일 열람을 통해 비밀 번호확인이 가능함
    ...
    4. 인증성
        ...
        5. 반복된 인증시도 제한 기능 미제공 - M: [로그인] 잘못된 비밀번호를 입력하여 로그인 시도를 5회 이상 반복하였지만 로그인계정이 잠기지 않음
...
7. 유지보수성
    ...
    3. 시험성
        - 자체 시험 기능을 제공하는 경우, 해당 기능이 정상 동작하는지 확인 → 연결 테스트 기능 오류
...
8. 이식성 : 설치가 잘 되고 다양한 환경에 적응하는지
    ...
    3. 대체성
        ...
        1. 기능 포괄성 부족 - M: 시험대상제품(v2.0)에서는 이전버전(v1.0)에서 제공하였던 ‘AAA’ 기능이 제공되지 않지만 이에 대한 설명이 매뉴얼에 제공되지 않음""",
            output_example="""기업 선택 기능 오류\tM\tA\t기능적합성\t"[그룹 관리] 기업명이 동일한 기업(기업코드는 다름)이 2개 이상 존재할 경우, 그룹 생성 시 기업 선택 드롭다운 목록에서 선택한 기업(기업코드: TEST3)이 아닌 동명의 다른 기업(기업코드: CP001)이 잘못 선택됨"
잘못된 식별 정보 제공\tM\tA\t일반적 요구사항\t"[시험대상 프로그램] [사용자 매뉴얼] 프로그램과 사용자 매뉴얼에 제품 식별 정보(예: 제품명, 버전)가 잘못 표시됨
